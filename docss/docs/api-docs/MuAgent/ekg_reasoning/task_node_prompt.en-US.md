...
group:
  title: EKG Reasoning
  order: 6
title: Task Node Prompt
order: 3
toc: content
---

### This document primarily introduces different prompts used in three modes: react, planning, and parallel.

The prompts here mainly define the output format, separating them from the prompts originally written in the nodes. The 'example' section contains few-shot content, providing several examples of the output format. The 'notes of caution' section outlines potential errors that large language models might originally encounter. This prompt design significantly reduces the likelihood of errors in the output format.

### 1. Prompt for the output format in react mode
...
### 2. Prompt for the output format in planning mode
...
_ task_description (str):     为行动方本轮的任务
...
### 3. Prompt for the output format in parallel mode
