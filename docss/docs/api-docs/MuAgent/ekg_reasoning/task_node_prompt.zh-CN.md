...
### 这个文档主要介绍了 react、planning、parallel 三种模式中不同的 prompt。

这里的 prompt 主要规定了输出了格式，分离出了原本写在节点中的 prompt。example 中是 few-shot 的内容，给出了输出格式的几个例子，注意事项中则给出了大语言模型原本可能出现的出错，这样的 prompt 设计很大程度上减少了输出格式出错的可能。

### 1. react 模式输出格式的 prompt
