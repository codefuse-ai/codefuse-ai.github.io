...
| substr                 | (string, int, int) -> string    | 通过初始 index 和 length 来截取字符串。                                                                                                                 |
...
2. `=`符号在 GödelScript 中是比较特殊的符号，不能用常规的编程语言的思路来理解。GödelScript 是一种 Datalog 语言。在这里，`=`符号同时具备两种语义，一个是 **赋值** 一个是 **判等**。详情可看[`=`运算符](#赋值和判等运算符)。
...
与其他语言类似，GödelScript 允许通过完整路径的方式直接指定一个符号，但是该符号必须被引入。
